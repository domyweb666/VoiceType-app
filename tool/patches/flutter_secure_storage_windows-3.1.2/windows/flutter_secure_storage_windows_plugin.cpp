...
namespace
{
  ...
  inline std::wstring Utf8ToWideStr(const char *utf8) {
    if (utf8 == nullptr || utf8[0] == '\0') {
      return L"";
    }
    int n = MultiByteToWideChar(CP_UTF8, 0, utf8, -1, nullptr, 0);
    if (n <= 0) {
      return std::wstring(utf8, utf8 + strlen(utf8));
    }
  ...
  inline std::string WideToUtf8Str(LPCWSTR wide) {
  ...
  const std::wstring kCredentialFilterW =
      Utf8ToWideStr((ELEMENT_PREFERENCES_KEY_PREFIX + '*').c_str());
  ...
  PBYTE FlutterSecureStorageWindowsPlugin::GetEncryptionKey()
  {
      ...
      const std::wstring target_name =
          Utf8ToWideStr(("key_" + ELEMENT_PREFERENCES_KEY_PREFIX).c_str());
  ...
  std::optional<std::string> FlutterSecureStorageWindowsPlugin::Read(const std::string &key)
  {
      ...
      if (!fs.good()) {
          //Backwards comp.
          PCREDENTIALW pcred;
          const std::wstring target_name = Utf8ToWideStr(key.c_str());
          bool ok = CredReadW(target_name.c_str(), CRED_TYPE_GENERIC, 0, &pcred);
          if (ok)
          {
...
#pragma warning(pop)
  ...
  flutter::EncodableMap FlutterSecureStorageWindowsPlugin::ReadAll()
  {
    ...
    bool ok = CredEnumerateW(kCredentialFilterW.c_str(), 0, &cred_count, &pcreds);
    if (!ok)
    {
        return creds;
    }
    ...
    {
      auto pcred = pcreds[i];
      std::string target_name = WideToUtf8Str(pcred->TargetName);
      auto val = std::string((char*)pcred->CredentialBlob);
  ...
  void FlutterSecureStorageWindowsPlugin::DeleteAll()
  {
    ...
    bool read_ok = CredEnumerateW(kCredentialFilterW.c_str(), 0, &cred_count, &pcreds);
    if (!read_ok)
    {
      auto error = GetLastError();
      if (error == ERROR_NOT_FOUND)
